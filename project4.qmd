...
format: 
  html:
    ...
    embed-resources: true   # ✅ makes Plotly interactive
execute:
  echo: false   # hide code, only show visuals
  warning: false
  error: false
---
...
We apply clustering, regression, and classification to evaluate trends in job markets, focusing on **salary, experience, and employability**.  
...
# -----------------
df_model = pd.get_dummies(df, columns=["industry"], drop_first=True)
X = df_model.drop(columns=["TITLE_NAME","salary","is_ai"]).select_dtypes(include=["int64","float64","uint8"])
y = df_model["salary"]
...
fig_reg = px.bar(reg_results, x="Metric", y="Value",
                 title="Regression Performance: Salary Prediction", text_auto=True)
fig_reg.show()

# -----------------
# Classification: AI vs Non-AI Jobs
# -----------------
df_clf = pd.get_dummies(df, columns=["industry"], drop_first=True)
X = df_clf.drop(columns=["TITLE_NAME","salary","is_ai"]).select_dtypes(include=["int64","float64","uint8"])
y = df_clf["is_ai"]
...
if len(y.unique()) > 1:
    ...
    fig_clf = px.bar(clf_results, x="Metric", y="Value",
                     title="Classification Performance: AI vs Non-AI", text_auto=True)
    fig_clf.show()
...
fig2 = px.scatter(features, x="salary", y="years_experience",
                  color="cluster", title="KMeans Clusters of Jobs (by Salary & Experience)")
fig2.show()

# -----------------
# Salary Distribution
# -----------------
fig1 = px.histogram(df, x="salary", color="is_ai", barmode="overlay",
...
Experience matters: more years of experience → higher salary clusters.
...
Target industries where AI adoption is accelerating.

Use clustering insights to position yourself in AI-heavy vs. traditional roles.
