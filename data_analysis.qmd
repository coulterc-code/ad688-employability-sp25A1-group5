...
## Create Derived Columns (before any dropping)
```{python}
# INDUSTRY_DISPLAY: pick the first non-null across many possible industry/NAICS name fields
INDUSTRY_CANDIDATES = [
...
# SALARY_DISPLAY: try SALARY → midpoint(SALARY_FROM/TO) → PAY_RATE annualized
salary_series = None
...
if df["SALARY_DISPLAY"].notna().any():
    df["SALARY_DISPLAY"] = df["SALARY_DISPLAY"].fillna(df["SALARY_DISPLAY"].median())
...
```{python}
# Compute top 15 industries once so both charts align
top15 = (
    df["INDUSTRY_DISPLAY"]
...
top15_list = top15["Industry"].tolist()
print("Top 15 industries selected:", top15_list[:5], "...")  # brief peek
```
...
if df["INDUSTRY_DISPLAY"].notna().sum() == 0:
    ...
    # Top 15 industries by postings ONLY
    counts = (
        df["INDUSTRY_DISPLAY"]
        .value_counts(dropna=False)
        .head(15)
    ...
    fig = px.bar(
        counts,
        x="Count",          # ✅ x-axis is just the numeric count
        y="Industry",
        orientation="h",
    ...
    fig.update_layout(
        xaxis=dict(title="Count", tickformat=","),
        yaxis=dict(title="Industry")
    )
    fig.show()
```
## Salary Distribution by Industry (Top 15 only)
```{python}
valid = df.dropna(subset=["INDUSTRY_DISPLAY","SALARY_DISPLAY"])
valid = valid[(valid["SALARY_DISPLAY"] > 0) & (valid["INDUSTRY_DISPLAY"].isin(top15_list))]
...
if valid.empty:
    ...
    fig2 = px.box(
        ...
        title="Salary Distribution by Industry (Top 15 by Postings)", points=False
    )
