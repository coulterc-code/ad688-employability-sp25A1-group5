...
```{python}
# Keep the most current NAICS/SOC name columns; drop legacy & metadata
columns_to_drop = [
    "ID","LAST_UPDATED_TIMESTAMP","DUPLICATES","ACTIVE_URLS","ACTIVE_SOURCES_INFO",
    "TITLE_RAW","BODY","COMPANY_RAW","URL","SOURCES","SOURCE_TYPES",
...
# Normalize salary-related fields
...
```{python}
# Robust column pickers (expanded)
INDUSTRY_CANDIDATES = [
    ...
    "LIGHTCAST_SECTORS_NAME","INDUSTRY","INDUSTRY_NAME","NAICS_2022_NAME","SECTOR_NAME"
]
SALARY_CANDIDATES = [
    "SALARY","SALARY_MEDIAN","SALARY_MID","SALARY_ANNUAL","PAY_RATE",
...
if (industry_col is None) or (salary_col is None):
    ...
    # if we have SALARY_FROM/TO but not a single salary, make a midpoint
    if salary_col in ["SALARY_FROM","SALARY_TO"] and {"SALARY_FROM","SALARY_TO"}.issubset(df.columns):
        df["_SALARY_MID"] = pd.to_numeric(df["SALARY_FROM"], errors="coerce")
        hi = pd.to_numeric(df["SALARY_TO"], errors="coerce")
        df.loc[:, "_SALARY_MID"] = (df["_SALARY_MID"] + hi) / 2
        salary_col_use = "_SALARY_MID"
    else:
        salary_col_use = salary_col

    sdf = df[[industry_col, salary_col_use]].copy()
    sdf.loc[:, salary_col_use] = pd.to_numeric(sdf[salary_col_use], errors="coerce")
    sdf = sdf.dropna(subset=[salary_col_use])
    sdf = sdf[sdf[salary_col_use] > 0]

    if sdf.empty:
        print("No salary data available for boxplot after cleaning.")
    else:
        fig2 = px.box(
            sdf, x=industry_col, y=salary_col_use,
